...
class Population:
    def __init__(self, size):
        ...
        self.species = []
        self.size = size
        self.max_lifespan = 0  # Track the maximum lifespan
    ...
    def natural_selection(self):
        print('SPECTATE')
        self.update_max_lifespan()  # Update max lifespan before creating the next generation
        self.speciate()
    ...
    def update_max_lifespan(self):
        for p in self.players:
            if p.lifespan > self.max_lifespan:
                self.max_lifespan = p.lifespan
